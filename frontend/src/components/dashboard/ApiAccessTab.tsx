        ...
        <Card className={`max-w-2xl mx-auto p-4 md:p-8 ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200'} backdrop-blur-sm rounded-2xl border shadow-xl relative overflow-hidden group`}>
            ...
            <div className="relative z-10 space-y-8">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 className="text-2xl md:text-3xl font-black">API Access</h2>
                        <p className={`text-sm md:text-base ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Integrate the AI agent into your own applications</p>
                    </div>
                </div>

                <div className="space-y-8">
                    <div className={`p-4 md:p-8 rounded-3xl border-2 border-dashed ${isDark ? 'border-gray-800 bg-black/20' : 'border-gray-200 bg-gray-50/50'} flex flex-col items-center justify-center text-center space-y-6 transition-all hover:border-purple-500/30`}>
                        {apiToken ? (
                            <div className="w-full space-y-6">
                                <Label className="text-sm font-bold uppercase tracking-wider opacity-70">Your API Token</Label>
                                <div className={`flex flex-col md:flex-row gap-3 p-4 rounded-2xl border ${isDark ? 'bg-black/40 border-gray-700' : 'bg-white border-gray-200'} shadow-inner`}>
                                    <code className="flex-1 text-xs md:text-sm font-mono truncate pt-1.5">{apiToken}</code>
                                    <Button size="icon" variant="ghost" onClick={() => copyToClipboard(apiToken)} className="h-10 w-10 hover:bg-purple-500/10 transition-colors self-end md:self-center">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative group">
                            <pre className={`p-4 md:p-6 rounded-2xl text-[10px] md:text-sm font-mono overflow-x-auto ${isDark ? 'bg-black/60 border-gray-800' : 'bg-gray-900 text-gray-100'} border shadow-2xl custom-scrollbar`}>
                                {`curl -X POST ${BACKEND}/agent/${activeAgent} \\
